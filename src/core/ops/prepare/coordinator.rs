...
pub(crate) fn prepare_mount_plan_with_root(
    ...
    if !vfs_ids.is_empty() {
        crate::scoped_log!(
            info,
            "prepare:vfs",
            "selected: backend={}, modules={}, operations={}, max_branches={}",
            capabilities.vfs_fs_type().unwrap_or("unknown"),
            vfs_ids.len(),
            vfs_ops.len(),
            capabilities.vfs_max_branches()
        );
    }
    if !vfs_fallback_ids.is_empty() && capabilities.vfs_status() != "disabled" {
        crate::scoped_log!(
            warn,
            "prepare:vfs",
            "fallback to overlay: modules={}, reason=capability_or_partition_safety",
    ...
    for module in modules {
        if vfs_ids.contains(&module.id) {
            crate::scoped_log!(
                debug,
                "prepare:vfs",
                "module bypasses copy planner: id={}, source={}",
                module.id,
    ...
    crate::scoped_log!(
        info,
        "prepare",
        "complete: vfs_ops={}, vfs_modules={}, vfs_fallback_modules={}, overlay_ops={}, overlay_modules={}, magic_modules={}, kasumi_modules={}, elapsed_ms={}, copied_entries={}, copied_bytes={}",
        plan.vfs_ops.len(),
